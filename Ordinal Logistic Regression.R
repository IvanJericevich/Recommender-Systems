...
setwd("")
# load("Regression Workspace.RData") # Load data from previous workspace
list_of_packages = c("MASS") # This automatically installs and loads packages not already installed on the users computer
new_packages = list_of_packages[!(list_of_packages %in% installed.packages()[, "Package"])] # MASS = polr
...
### 3. Ordinal Logistic Regression
model = polr(rating ~ . - movieId - y2010s, data = train)
predicted = predict(model, newdata = test, type = "class")
predicted = as.numeric(levels(predicted))[predicted]
actual = as.numeric(levels(test$rating))[test$rating]
MAE(actual, predicted)
RMSE(actual, predicted)
tab = table(ratings$rating, predicted)
misclass = 1 - sum(diag(tab)) / sum(tab)
####################################################################################################

### 4. Save Workspace
rm()
save(list = c(""), file = paste0(getwd(), "/Data/Regression Workspace.RData"))
save.image(paste0(getwd(), "/Data/Regression Workspace.RData"))
####################################################################################################
